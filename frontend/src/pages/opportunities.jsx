...
const Opportunities = () => {
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-indigo-500 selection:text-white relative overflow-hidden pb-20">
      ...
      <main className="pt-36 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto relative z-10 animate-fade-in-up opacity-0" style={{ animationFillMode: 'forwards' }}>
        <div className="text-center mb-16">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-white tracking-tight mb-4">
            Discover <span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-indigo-400">Opportunities</span>
          </h1>
          <p className="text-slate-400 text-lg max-w-2xl mx-auto">
            Whether you're looking for a permanent career, a quick freelance gig, or a passion project, we have it all categorized for you.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {opportunityTypes.map((type, index) => (
            ...
            >
              <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${type.color} flex items-center justify-center text-white mb-6 shadow-lg transform group-hover:scale-110 group-hover:rotate-3 transition-all duration-300`}>
                {type.icon}
              </div>
              ...
              <Link to={`/opportunities/${type.title.toLowerCase()}`} className="inline-flex items-center text-sm font-semibold text-slate-300 hover:text-white transition-colors group/link mt-auto">
